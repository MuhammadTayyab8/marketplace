...
export default function Products() {
  ...
  const totalPages = Math.ceil(products.length / productsPerPage);
   ...
   const endProduct = Math.min(indexOfLastProduct, products.length);
 ...
 <div className=" flex flex-wrap justify-between items-center bg-[#F9F1E7] w-full px-4 md:px-20 py-5 gap-4">
        <div className="flex items-center flex-wrap gap-2 md:gap-4">
          ...
          <span className="border-l border-black pl-2 sm:pl-4">
            <p className="text-[12px] md:text-base"> Showing {startProduct} - {endProduct} of {products.length} results</p>
          </span>
        </div>
        ...
        <div className="fixed inset-0 z-40 bg-black bg-opacity-50 flex justify-start">
          <div className="bg-white w-80 p-6 space-y-4 transform transition-transform duration-300 ease-in-out overflow-y-auto max-h-full">
            ...
            <div className="space-y-2">
              <div className="flex flex-col justify-between pb-2 mb-2">
                {['Home Decor', 'craftsmanship', 'Luxury', 'chair', 'modern', 'fancy'].map(
                  (category) => (
                    <div
                      key={category}
                      className={`flex justify-between items-center pb-2 mb-2 text-gray-600 cursor-pointer ${
                        selectedTags.includes(category) ? 'text-black font-bold' : ''
                      }`}
  ...
  {loading ? (
    ...
    <>
      {currentProducts.filter(
        product =>
          // Filter by selected condition (e.g., "New" or "Used")
          (!selectedCondition ||
            (selectedCondition === "NEW" && product.isNew) ||
            (selectedCondition === "USED" && !product.isNew)) &&
      ...
      <div className="flex justify-center items-center py-5 gap-4 cursor-pointer pb-8">
        ...
        <span className="flex items-center justify-center">
          <p
            className={`w-20 h-10 flex items-center justify-center rounded-lg cursor-pointer ${
              currentPage === totalPages ? 'bg-[#B88E2F] text-white' : 'bg-[#F9F1E7] text-black'
            }`}
